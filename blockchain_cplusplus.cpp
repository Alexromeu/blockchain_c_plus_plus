...
std::string Blockchain::calculateHash(const Block& block) {
    std::string hashInput = std::to_string(block.index) + block.previousHash + transaction_to_str(block.data) + std::to_string(block.timestamp) + std::to_string(block.nonce);
...
void Blockchain::addBlock(Block &newBlock) {
    newBlock.previousHash = this->chain.back().hash;
...
int main() {
    ...
    Block newBlock;
    newBlock.index = 1;
    newBlock.data = {transaction1, transaction2};

    
   myBlockchain.printChain();
    myBlockchain.printBlock(newBlock);
